...
Имеется таблица T с учениками, 2 поля (ФИО точно идентифицирует
ученика):

Необходимо:
1) Определить количество двоек для учеников, у которых 10 и больше
пятерок
2) Вывести количество оценок, полученное по каждому предмету каждым
учеником по отдельности */
...
-- Вопрос 2
/* 1) Найти тип первой и тип последней операции для каждого клиента и
вывести их с их датами.
Желательно использовать один запрос без подзапросов с двумя
единственными оконными функциями – по одной на максимум и минимум.
2) Найти для каждой операции количество дней с даты последней оплаты
мобильного (операция «Оплата мобильного»). Если тип операции «Оплата
мобильного», то кол-во дней равно 0.
*/
CREATE TABLE Op (
  CUSTOMER_RK INT,
...
SELECT 
    ...
    OPERATION_DTTM,
    OPERATION_TYPE_CD,
    DATEDIFF(DAY, MAX(CASE WHEN OPERATION_TYPE_CD = 'Оплата мобильного' THEN OPERATION_DTTM END) 
             OVER (PARTITION BY CUSTOMER_RK ORDER BY OPERATION_DTTM),
             OPERATION_DTTM) AS Days
FROM  Op
GROUP BY CUSTOMER_RK, OPERATION_DTTM;
...
Преимущества такого подхода: 
 ...
 - происходит дублирование данных и впустую расходуется дисковое пространство. Например, у пользователя Еленина Елена Еленовна баланс не менялся на протяжении 5 дней, 
...
Преимущества такого подхода: 
 - максимальная простота работы с таблицей
...
Недостатки такого подхода:
 - сложность при работе с таблицей - например, при изменении баланса требуется выполнять два запроса 
 (update для изменения предыдущей записи и insert для вставки новой записи)

SCD2 является наиболее оптимальным методом хранения подобных данных, так как являет собой компромисс между занимаемым дисковым пространством и простотой работы с таблицей.
...
SCD2 подразумевает использование двух полей для хранения дат, определяющих временной интервал актуальности записи. 
...
Напишем запрос на выборку данных, который будет формировать ответ в виде SCD2: */
